...
if __name__ == '__main__':
    ...
    def real():
        ...
        boi = LinearRegressionARD(polydegree=1, n_gibbs=2000)
        boi.fit(X, y)
        fig, _ = boi.plot_w_traces(axes=np.arange(15), n_cols=5, ticks=True, 
                          show=False)
        fig.suptitle('ARD')
        # plt.savefig('images/real_w_traces_ard.pdf')
        plt.close()

        fig, _ = boi.plot_alpha_traces(axes=np.arange(15), n_cols=5, 
        ...
        print(boi.get_mae())
        argsort = boi.alpha_hat.argsort()[::-1]
        # print(argsort)
        # print(boi.alpha_hat[argsort])
        
        boi = LinearRegression(polydegree=1, n_gibbs=2000)
        boi.fit(X, y)
        fig, _ = boi.plot_w_traces(axes=np.arange(15), n_cols=5, ticks=True, 
                                   show=False)
