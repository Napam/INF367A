...
class BaseMCMCFactorizer:
    ...
    def fit(self, X: np.ndarray):
        '''
        This abstractfunction should create the instance attributes "Us" 
        and "Vs"
        '''
    ...
    def _plot_ci(self, n, P, lower_bounds, upper_bounds, ax=None, *args, 
                 **kwargs):
        '''
...
class SimpleFactorizer(BaseMCMCFactorizer):
    '''
    ...
    def fit_transform(self, df: pd.DataFrame, **kwargs):
        ...
        self.Us: np.ndarray, self.Vs: np.ndarray, self.lp__: np.ndarray =\
    ...
    def ci(self, n_elements: int=20, n_samples: int=1000, p=0.95, 
        ...
        P = np.random.normal(loc=P, scale=self.sigma_x, size=P.shape)
        P.sort(axis=0)
        ...
        if show:
            self._plot_ci(n_elements, P, lower_bounds, upper_bounds, *args, 
                          **kwargs)
